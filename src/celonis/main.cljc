(ns celonis.main
  "Kotodama WASM entrypoint for the Celonis clean-room actor (L4) — Clojure port.
  ...
  facts (`celonis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "celonis")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"celonis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "celonis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :celonis-compat/ready)
